...
class ChatService {
  ...
  async sendMessage(message: string): Promise<string> {
    ...
    try {
      ...
      if (user) {
        databaseService.logMessage(user.name, user.phone, message, 'text', 'AI_ASSISTANT');
      }
      ...
      // Log AI Response
      if (user) {
        databaseService.logMessage('AI_ASSISTANT', 'SYSTEM', result.text, 'text', user.phone.replace(/\D/g, ''));
      }
