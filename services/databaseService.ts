...
import { doc, setDoc, serverTimestamp, collection, addDoc, getDocs, query, orderBy, deleteDoc, getDoc, onSnapshot, writeBatch, updateDoc, where, getDocFromServer } from 'firebase/firestore';
...
export const databaseService = {
  ...
  logActivity: async (user: User, action: string, extra?: any) => {
    try {
  ...
  logCentralizedRequest: async (user: User, type: 'worker' | 'equipment' | 'real_estate' | 'emergency' | 'location', data: any) => {
    try {
      await databaseService.ensureAuth();
      const requestRef = collection(db, 'centralized_requests');
      const requestEntry = {
        userId: auth.currentUser?.uid || null,
        userName: user.name,
      ...
      await addDoc(requestRef, requestEntry);
      console.log(`Centralized request logged: ${type} for ${user.name}`);
    } catch (e) {
      handleFirestoreError(e, OperationType.WRITE, 'centralized_requests');
    }
  },

  logMessage: async (senderName: string, senderPhone: string, content: string, type: 'text' | 'image' | 'audio' = 'text', receiverId: string = 'AI_ASSISTANT') => {
    try {
      await databaseService.ensureAuth();
      const msgRef = collection(db, 'message_history');
      const msgEntry = {
      ...
      await addDoc(msgRef, msgEntry);
    } catch (e) {
      console.error("Error logging message history:", e);
    }
  },
  ...
  logConnection: async (user: User) => {
    try {
        ...
        databaseService.logActivity(user, 'connect');
    } catch (e) {
  ...
  syncUserToFirestore: async (user: User) => {
    ...
    try {
      ...
      try {
        ...
        handleFirestoreError(e, OperationType.WRITE, `users/${sanitizedPhone}`);
      }
    } catch (e) {
      console.error("Error in syncUserToFirestore:", e);
      // Don't throw here to avoid blocking the main flow if it's just a non-critical sync
    }
  },
  ...
  getUserFromFirestore: async (name: string, phone: string): Promise<User | null> => {
    ...
    try {
      ...
      handleFirestoreError(e, OperationType.GET, `users/${sanitizedPhone}`);
    }
    return null;
  },
  ...
  loginUser: async (phone: string, pin: string): Promise<{user: User | null, error?: string}> => {
    ...
    // --- Special ADMIN Recognition ---
    if (normalizedInputPhone === '0705052632' && pin === '0661') {
        console.log("Admin login detected");
        // We look for admin in Firestore first
        let adminUser = await databaseService.getUserByPhoneFromFirestore(normalizedInputPhone);
        
        if (!adminUser) {
            // Auto-create admin if missing from DB
            adminUser = {
                name: 'Mael',
                phone: '0705052632',
                city: 'Cocody',
                pin: '0661',
                userId: auth.currentUser?.uid || 'ADMIN_AUTO',
                role: 'Admin',
                isVerified: true,
                lastConnection: new Date().toISOString()
            } as User;
            ...
            // Ensure admin has correct name/city in local memory if they changed
            adminUser.name = 'Mael';
            adminUser.city = 'Cocody';
    ...
    if (firestoreUser) {
        ...
        // Allow Session Takeover - If PIN is correct, we trust the user
        ...
        // Update Firestore session and sync
    ...
    return { user: null, error: "Compte non trouvé. Veuillez vous inscrire si c'est votre première fois." };
  },
  ...
  registerUser: async (name: string, city: string, phone: string, pin: string): Promise<{user: User | null, error?: string}> => {
    ...
    // 1. Check Firestore (Source of truth) - STRICT NO DUPLICATE
    console.log("Checking if user already exists in Firestore before registration:", normalizedPhone);
    try {
      const existingFirestoreUser = await databaseService.getUserByPhoneFromFirestore(normalizedPhone);
      if (existingFirestoreUser) {
          return { user: null, error: "Ce numéro est déjà lié à un compte existant. Veuillez vous connecter au lieu de vous réinscrire." };
      }
    } catch (e) {
      console.warn("Firestore check failed during registration, proceeding with precaution:", e);
    }

    // 2. Check localStorage (Legacy/Offline)
    const localUser = users.find(u => u.phone === normalizedPhone);
    if (localUser) {
        return { user: null, error: "Ce numéro est déjà utilisé sur cet appareil. Veuillez vous connecter." };
    }
  ...
  cleanupDatabase: async () => {
    ...
    try {
      ...
      // 2. Ensure Admin exists with PIN 0661
      ...
      await setDoc(adminRef, {
        ...adminData,
        userId: auth.currentUser?.uid || 'ADMIN_SYSTEM',
        updatedAt: serverTimestamp(),
        lastSeen: serverTimestamp()
      }, { merge: true });
      
      console.log("Admin account verified/created with PIN 0661.");
  ...
  getWorkers: async (): Promise<Worker[]> => {
    ...
    // Attempt to fetch from Client-side Firestore first for real data
    try {
      ...
      snapshots.forEach((snapshot, index) => {
        ...
        snapshot.docs.forEach(doc => {
          ...
          allWorkers.push({
            id: doc.id,
            name: data.jobTitle || data.ownerName || data.agencyName || data.companyName || 'Inconnu',
            description: data.description || data.experience || '',
            category: type === 'travailleurs' ? 'Individuel' : (type === 'agences' ? 'Agence' : (type === 'entreprises' ? 'Entreprise' : 'Matériel')),
            profileImageUrl: data.profileImageUrl || '',
            phone: data.phone || '',
            rating: data.rating || 4.0,
            ville: data.city || data.location || ''
          });
        });
      });
      ...
      console.warn("Client-side direct fetch failed, falling back to API:", e);
    }
    ...
    try {
      ...
      console.error("Failed to fetch workers from API", e);
    }
  ...
  onUnreadMessagesCount: (chatUserId: string, callback: (count: number) => void) => {
    ...
    const path = `chats/${userId}/messages`;
    const q = query(
  ...
  saveAdminChatMessage: async (chatUserId: string, message: any) => {
    ...
    try {
      ...
      await setDoc(doc(db, 'chats', userId), {
        ...
        updatedAt: serverTimestamp(),
        userId: userId, // Keep phone for legacy
        ownerUid: auth.currentUser?.uid || null // Store the actual UID for rules
      }, { merge: true });
      return true;
    } catch (e) {
      handleFirestoreError(e, OperationType.WRITE, path);
  ...
  saveFormSubmission: async (formData: any) => {
    try {
      ...
      const userPhone = formData.userPhone || activeUser?.phone;
      const userName = formData.userName || activeUser?.name || 'Utilisateur';

      // 1. Log the submission to Firestore
      ...
      // 2. Automate Chat Messages if phone is available
      if (userPhone) {
        const sanitizedPhone = userPhone.replace(/\D/g, '');
        const formTitle = formData.formTitle || 'Demande';
        const totalPrice = formData.totalPrice || 310;
        
        // Log centralized request if needed
        if (activeUser) {
          await databaseService.logCentralizedRequest(activeUser, formData.formType || 'worker', formData);
        }

        // Send User Summary Message to Assistant Chat
        const summaryText = formData.whatsappMessage || `Nouvelle demande: ${formTitle}`;
        await databaseService.saveAdminChatMessage(sanitizedPhone, {
          text: summaryText,
          sender: 'user',
          timestamp: Date.now(),
          isRead: false
        ...
        // Send Assistant Confirmation Message with Buttons
        const confirmationText = "Merci, nous avons bien reçu votre demande. Un agent va vous contacter. Veuillez payer les frais de mise en relation.";
        await databaseService.saveAdminChatMessage(sanitizedPhone, {
          text: confirmationText,
          sender: 'admin', // or 'assistant', ChatScreen expects 'admin' for incoming messages
          timestamp: Date.now() + 500, // Slightly later
          isRead: false,
          paymentInfo: {
            amount: totalPrice,
            reason: 'Frais de mise en relation'
          },
          whatsAppPayload: {
            phone: "+2250705052632",
            text: summaryText // Send the full summary via WhatsApp as requested
          }
        });
      }
      ...
      handleFirestoreError(e, OperationType.WRITE, 'form_submissions');
      return false;
    }
  },
  ...
  getCentralizedRequestsAdmin: async () => {
    try {
      const q = query(collection(db, 'centralized_requests'), orderBy('timestamp', 'desc'));
      const snapshot = await getDocs(q);
      return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    } catch (e) {
      handleFirestoreError(e, OperationType.LIST, 'centralized_requests');
      return [];
    }
  },

  getActivityLogsAdmin: async () => {
    try {
  ...
  updateOfferBlur: async (offerId: string, isUnblurred: boolean) => {
    try {
      ...
      await updateDoc(offerRef, { isUnblurred, updatedAt: serverTimestamp() });
      return true;
    } catch (e) { return false; }
  },

  publishStatusAsMessage: async (userPhone: string, statusText: string) => {
    try {
      await databaseService.saveAdminChatMessage(userPhone, {
        text: statusText,
        sender: 'user',
        timestamp: Date.now(),
        isRead: false
