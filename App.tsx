...
const App: React.FC = () => {
  ...
  const [menuView, setMenuView] = useState<'hub' | 'worker_list' | 'notifications' | 'emergency_form' | 'assistant_qr' | 'tally_form'>('hub');
...
// Card data and role sync logic removed
  ...
  if (isAdminDashboardOpen && isUserAdmin) {
    return <AdminDashboard onBack={() => setIsAdminDashboardOpen(false)} />;
  }
    ...
    case Tab.Menu:
      switch (menuView) {
        case 'worker_list':
          activeScreen = <WorkerListScreen 
            ...
            onScheduleService={(url, title) => {
                if (currentUser) {
                  databaseService.logCentralizedRequest(currentUser, 'worker', {
                ...
                if (url) {
                    setTallyUrl(url);
                    setTallyTitle(title || "Service FILANT°225");
                    navigateTo({ activeTab: Tab.Menu, menuView: 'tally_form' });
                } else {
        ...
        case 'assistant_qr':
          ...
          break;
        case 'tally_form':
          activeScreen = <TallyFormScreen formUrl={tallyUrl} formTitle={tallyTitle} onBack={handleBack} user={currentUser} />;
          break;
        case 'hub':
        default:
