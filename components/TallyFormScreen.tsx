...
const TallyFormScreen: React.FC<TallyFormScreenProps> = ({ formUrl, formTitle = "Service FILANT°225", onBack, user }) => {
  useEffect(() => {
    if (user) {
      databaseService.logActivity(user, 'tally_form_open', { 
        formUrl, 
        formTitle 
      }).catch(console.error);
    }
  }, [user, formUrl, formTitle]);
